...
import { completePlan } from '../../hooks/usePlans';
import { todayISO } from '../../utils/dates';
import { showToast } from '../shared/Toast';
import type { MaintenanceType } from '../../types';
import type { PlanView } from '../../hooks/usePlans';

interface CompletePlanFormProps {
  plan: PlanView | null;
  open: boolean;
  onClose: () => void;
}

export default function CompletePlanForm({ plan, open, onClose }: CompletePlanFormProps) {
  ...
  useEffect(() => {
    if (open && plan) {
      setDate(plan.date <= todayISO() ? todayISO() : plan.date);
      setNotes(plan.notes);
    }
  }, [open, plan]);
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/40 animate-fade-in" onClick={onClose}>
      ...
      >
        ...
        <p className="text-sm text-gray-500 mb-4">{plan.customerName}</p>
